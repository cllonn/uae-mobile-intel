...
- **GenAI / Grounded Copilot: Routing + tool layer validated (T6), LLM narration pending.**
  `src/copilot_tools.py` and `src/copilot.py` are a deterministic tool layer and a keyword-based
  question router with a template-only narrator (no LLM connected yet). `tests/test_t6_copilot_
  evaluation.py` runs 72 questions (the brief's categories, including the mandatory e&-attribution
  trap) through the real routing/tool/narration path end to end and scores factual correctness,
  evidence grounding, hallucination, and appropriate uncertainty – **72/72 passing**
  (`docs/t6_copilot_evaluation.md`). What's still pending is connecting a real LLM for the
  generated-language narration path (the template narrator is what's evaluated today); see
  "Grounded copilot (prototype)" below for exactly what does and doesn't exist yet.
...
- `scripts/` – one-off/reusable utilities (emirate boundary extraction, the canonical-questions
  demo).
- `tests/` – reproducible validation scripts (T1/T3/T4/T5/T6).
- `docs/` – data dictionary, validation summary, T6 copilot evaluation, T7 usability protocol.
...
- **ML anomaly detection** (`src/anomaly_detection.py`) – two Isolation Forest models (Peer Gap:
  ...
  compared against a deterministic bottom-decile-Experience-Index-within-peer-group baseline, and
  against LOF as a second ML comparison. **T2 result, reported honestly**: on a blind
  synthetic-anomaly hold-out injecting all four of the brief's fault types – download -40%,
  latency +80%, gradual 3-quarter decline, and combined (both at once) –
  (`tests/test_t2_anomaly_benchmark.py`, reproducible, single fixed seed), Isolation Forest does
  not clearly beat the deterministic baseline, and per-fault-type recall against targets set in
  advance (before running the detector) is honest, not flattering: a single-fault signal (download
  *or* latency alone) is genuinely hard for this feature set to isolate. See
  `docs/validation_summary.md` for the full precision/recall/F1/FPR numbers per detector and per
  fault type. Kept as supplementary evidence, not presented as superior.
...
`tests/` holds reproducible T1/T2/T3/T4/T5/T6 scripts – all passing on the current
(30-test-threshold) pipeline output, including the honest T2 ML-vs-baseline result and the T6
copilot evaluation (72/72 questions, `python -m tests.test_t6_copilot_evaluation`).
`docs/validation_summary.md` has the full T0–T7 status and how to run them.
`docs/t6_copilot_evaluation.md` is the T6 copilot evaluation report (question bank, scoring
rules, and the 13 router/tool bugs it found and fixed).
...
2. Get mentor sign-off on the analytical methodology (Section "Status" above) before starting
   GenAI in earnest.
3. Connect a real LLM (`ANTHROPIC_API_KEY` + `pip install anthropic`) and re-run
   `tests/test_t6_copilot_evaluation.py` to additionally score the generated-language path (the
   72-question bank and grading rules are unchanged; only the narrator's `mode` becomes `"llm"`).
