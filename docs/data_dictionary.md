...
| `area_name` | Real local place name (e.g. `"Al Mafraq"`, `"Deira"`), or `"Unnamed area – <emirate>"` if no OSM place node was close enough | Nearest OSM `place=neighbourhood/suburb/quarter/village/town/hamlet/city` node to the H3 cell centroid, within a per-tier max distance – `scripts/build_area_name_field.py`. User-facing display label only; never used for scoring, joins, or map highlighting (those stay on `h3_cell`) |
| `area_name_source` | Which OSM `place` tag matched (`neighbourhood`, `suburb`, …), or `"fallback"` | Same script |
| `area_name_distance_km`, `area_name_is_fallback` | Distance to the matched place node, and whether no match was found at all | Same script |
...
### `peer_groups_uae.parquet`, `population_zones_uae.parquet`, `osm_density_zones_uae.parquet`, `area_names_uae.parquet`
...
inspected without re-deriving it from the joined table. `area_names_uae.parquet` covers the same
1,869-cell "measured or populated" union as `emirate_zones_uae.parquet` (see below), one row per
H3 cell: `h3_cell`, `area_name`, `area_name_source`, `area_name_distance_km`,
`area_name_is_fallback`. Built by `scripts/build_area_name_field.py`; 1,563 of 1,869 zones
(83.6%) resolved to a real OSM place name, 306 fell back to `"Unnamed area – <emirate>"`. Among
zone-quarter rows that actually get classified (evidence_tier `low`/`full`), 1,834 of 1,845
(99.4%) carry a real name – the fallback rate is far higher on the full measured-or-populated
universe than on the classified subset, because desert/industrial-edge cells with little Ookla
evidence are also the cells least likely to sit near a named OSM neighbourhood.
